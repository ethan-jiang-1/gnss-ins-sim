...
def _load_processed_data(path_name, route_name, route_num):
    ...
    fn_pos = "{}/ref_pos.csv".format(path_name)
    fn_ori = "{}/ref_att_quat.csv".format(path_name)
    ...
    acc_data = filter_gravity_in_acc(acc_data, gyro_data, src="sim_org_pure")
...
def _save_to_csv(route_name, route_num, df_imu, df_gt):
    ...
    data_root = "ds_Sim/{}_{}".format(route_name, route_num)
...
def do_split_sdir(name):
    print(name)
    path_name = "data_org/org_Sim/" + name
